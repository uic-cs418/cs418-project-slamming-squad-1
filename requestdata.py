...
class GetData:
    ...
    def parse_page_coindesk(self, html):
        driver = webdriver.Chrome()
        driver.get(self.url)
        # loadmore = driver.find_element_by_class_name("cn-section-controls")
        loadmore = driver.find_element_by_xpath("//div[@id='load-more-stories']/button[1]")
        # driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
        # driver.execute_script("arguments[0].scrollIntoView(true);", loadmore)
        driver.execute_script("arguments[0].scrollIntoViewIfNeeded(true);", loadmore)
        time.sleep(5)
        ...
        if url_next:
            ...
            url_next = None
        news = soup.find_all('a', class_="stream-article")
        news_list = []
        for n in news:
            title = n.find("h3").text
            date = n.find('time').get("datetime")[:10]
            # new = {'text': title, 'date': date}
            # news_list.append(new)

            link = n.get("href")
            r = requests.get(link)
            soup_in_bowl = BeautifulSoup(r.text, 'html.parser')
            article = soup_in_bowl.find('div', class_="entry-content")
            paragraphs = article.find_all('p')
            full_article = ""
            for p in paragraphs:
                full_article += " "
                full_article += p.text
            new = {'text': full_article, 'date': date}
            news_list.append(new)
...
def main():
    ...
    GetData(website_nickname="coindesk", url=url)
